...
<script type="text/javascript">
	...
	function saveInvoice() {
        ...
        $.ajax({
			url:"/supplier/saveInvoiceInfo.do",
			data:{
				"orderId" : orderId,
				"invoiceId" : invoiceId,
			...
			success:function(str){
				...
				if(result.state == 0){
					...
					//更新客户状态
                	 $.post("/supplier/port/sendCustomerStatus.do",
       	            	  { 
              		      "orderId" : orderId
       	            	  },
       	        	   function(data){
				    ...
				    setTimeout(show_bank,2000,invoiceId,orderId);	
				}else{
					easyDialog.open({
						  container : {
						    header : 'Prompt message',
	...
	function modifyProduct(index,productName,unitPrice,quantity,mold){
	...
	function updateProduct(){
		...
		var mold = $('#mold').val();
		var total = (parseFloat(unit_price) * parseFloat(quantity) + parseFloat(mold)).toFixed(2); 
		...
		$("#drawingBody tr:eq(" + index + ")").children("td:eq(3)").text(mold);
		$("#drawingBody tr:eq(" + index + ")").children("td:eq(4)").text(total);
		toSum();				
		$('#dialog').hide();
	}
	...
	<div id="content">
		<div id="content-header">
			...
			<div class="shu">
				<div class="bianhaolan">
					...
					<div class="fapiao">
						<label>发票编号 :</label> <span id="invoice_id">${invoiceId}</span>
					</div>
					<div class="jine">
						<label>金额单元 :</label> <select class="xiala_select" name="select"
							...
							<c:forEach var="amountUnit" items="${amountUnit}" varStatus="i">
								<option value="${amountUnit.id}">${amountUnit.currency}</option>
							</c:forEach>
						</select>
											...
											<tr class="gradeA odd" id="tr${i.index}">
												...
												<td class="center">
													<button class="btn btn-primary" 
														onclick="modifyProduct(${i.index},'${obj.productName}',${obj.unitPrice},${obj.quantity},${obj.moldPrice})">编辑</button>
		...
		<div class="container-fluid">
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" name="number" id="shipping_fee"
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" name="otherPrice" id="other_price"
						oninput="toSum();" onkeyup="keyUp(this)" onKeyPress="keyPress(this)" onblur="onBlur(this)"/><span>
						* 输入其他附加费用，如无默认为0（<span id="otherPrice_AmountUnit">美元</span>）</span>
				</div>
				<label class="control-label">其他费用备注: </label>
				<div class="controls">
					<textarea name="a" style="width:400px;height:80px;" id="comment"></textarea> <span>
						* 输入其他附加费用说明</span>
				</div>
			...
			<div class="control-group">
				...
				<select class="xiala_select" id="select_payment" onchange="select_remark()" style="margin-left: 10px;margin-top: 10px;">
				        ...
				        <option value="${obj.id}">${obj.remarkBank}</option>
				        </c:forEach>
			    </select>
				<div class="controls">				    
					<textarea name="a" style="width:800px;height:300px;" id="invoice_remark">${invoiceRemarks.size() == 0 ? "" : invoiceRemark.get(0).remark}</textarea> <span>
						* 银行付款信息备注</span>
				</div>
			...
			<div class="pull-right">
				<h4 style="float: right;">
					<span>总计:</span><span id="sum_price"></span>（<span id="sum_amountUnit">美元</span>）
					<input name="exchangeRate" id="exchangeRate" value="1.0" type="hidden">
				</h4>
				<br> <a class="btn btn-primary btn-large pull-right"
					style="margin-top: 15px; float: left;" id="next_btn" onclick="saveInvoice()">下一步</a>
			</div>
