...
public class QuotationUptakePrintTemplate {
	...
	public static String printExcel(HttpServletRequest request,String path,QuotationBean quotationBean,List<QuotationProductionBean> list) throws Exception{
		 ...
		 * 打开模板，复制sheet，另存
		 */
       File file = new File(path+"pdf"+File.separator+"quotation_uptake.xls");
		...
		HSSFWorkbook wb = new HSSFWorkbook(fi);
			...
			nCell.setCellValue("Client:"+quotationBean.getCustomerName());
			nCell = nRow.getCell(7);
			nCell.setCellValue("Quote on:"+quotationBean.getQuotationDate());
			
			nRow = sheet.getRow(rowNo++);
			nCell = nRow.getCell(7);
			nCell.setCellValue("Price valid:" +quotationBean.getQuotationValidity()+ "days");
			...
			if(tl == 1){
				...
				for (int i = 0;i < production_tl; i++) {	
					...
					nCell = nRow.getCell(4);
					nCell.setCellValue(list.get(i).getQuotationProductionPriceBeanList().get(0).getMaterialWeight()*1000);
